...
PosixApi::PosixApi(int port, PollingMethod method) : port(port), method(method), max_fd(0), epoll_fd(-1) {
    setupServer();
}
...
void PosixApi::setNonBlocking(int fd) {
...
void PosixApi::handleNewConnection() {
...
void PosixApi::handleClientData(int client_fd) {
    char buffer[1024];
