...
#ifdef ENABLE_LOG
#define LOG(_fmt, ...) fprintf(stdout, "[%s:%d]: %s"_fmt, __FILE__, __LINE__, __VA_ARGS__)
#else
...
namespace kvstore {
	...
	public:
		...
		virtual int modify(const char *key, const char *value) = 0;
		virtual int count() const = 0;
	};

	class ArrayKVStore : public KVStore {
	public:
		ArrayKVStore();
		~ArrayKVStore();
		...
		int del(const char *key) override;
		int modify(const char *key, const char *value) override;
	...
	private:
		...
		static constexpr int KVS_ARRAY_SIZE = 1024;
		KvsArrayItem *array_table_;
		int array_idx_;
	};
	...
	public:
		...
		int process_request(ConnItem &item);
		int init();
		void cleanup();

	private:
		ArrayKVStore array_store_;
		int split_token(char *msg, char **tokens);
		int parse_protocol(ConnItem &item, char **tokens, int count);
	};
