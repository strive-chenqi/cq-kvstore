...
//使用多进程方式
void upload_file_pipe(const char *confFile, const char *uploadFile, char *fileID, int size) {
    //1. 创建管道
    int fd[2];
    int ret = pipe(fd);
    if (ret == -1) {
...
//使用fastDFS API实现
int upload_file_api(const char *confFile, const char *myFile, char *fileID) {
